...
class CrossAttention(nn.Module):
    def __init__(self, x_dim, q_dim=512, num_heads=4):
        super().__init__()
        
        assert q_dim % num_heads == 0
        dim_head = q_dim // num_heads

        self.linear_kv = nn.Linear(x_dim, q_dim * 2)
        
        self.mlp = nn.Sequential(nn.Linear(q_dim, q_dim), nn.GELU(), nn.Linear(q_dim, q_dim))
        self.pre_norm = nn.LayerNorm(q_dim)
        self.post_norm = nn.LayerNorm(q_dim)
    ...
    def forward(self, q, x):
        ...
        q = rearrange(q, 'b (h d) -> b h 1 d', h=self.num_heads)
