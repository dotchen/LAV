...
def prettify_json(args):
    ...
    per_route_scores = defaultdict(lambda:defaultdict(list))
    ...
    if records_table:
        ...
        for route in records_table:
            ...
            for metric_key, metric_val, _ in metrics_route:
                per_route_scores[route['route_id']][metric_key].append(float(metric_val))

            for metric_key, metric_val, _ in infractions_route:
                num_occur = re.search(r'\((.*?) occurrences\)', metric_val).group(1)
                per_route_scores[route['route_id']][metric_key].append(float(num_occur) / route_completed_kms)
        ...
        total_route_metrics = defaultdict(lambda:defaultdict(list))

        for route_metrics in per_route_scores.values():
            for metric_key, metric_vals in route_metrics.items():
                for i, val in enumerate(metric_vals):
                    total_route_metrics[i][metric_key].append(val)

        global_route_metrics = defaultdict(list)
        for seed, route_metrics in total_route_metrics.items():
            for metric_key, metric_vals in route_metrics.items():
                global_route_metrics[metric_key].append(np.mean(metric_vals))

        for metric_key, metric_vals in global_route_metrics.items():
            list_statistics.extend([[metric_key, f'{np.mean(metric_vals):.2f} +/- {np.std(metric_vals,ddof=1):.2f}']])

        output += f'==== Analysis : {args.file} ===\n'
