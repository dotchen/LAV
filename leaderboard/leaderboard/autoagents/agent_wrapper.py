...
class AgentWrapper(object):
    ...
    allowed_sensors = [
        ...
        'sensor.pretty_map',                            # This is for visualization
    ]
    ...
    def setup_sensors(self, vehicle, debug_mode=False):
        ...
        for sensor_spec in self._agent.sensors():
            ...
            elif sensor_spec['type'].startswith('sensor.pretty_map'):
                delta_time = CarlaDataProvider.get_world().get_settings().fixed_delta_seconds
                frame_rate = 1 / delta_time
                sensor = PrettyMapReader(vehicle, frame_rate)
            elif sensor_spec['type'].startswith('sensor.speedometer'):
                delta_time = CarlaDataProvider.get_world().get_settings().fixed_delta_seconds
                frame_rate = 1 / delta_time
                sensor = SpeedometerReader(vehicle, frame_rate)
            elif sensor_spec['type'].startswith('sensor.objects'):
                delta_time = CarlaDataProvider.get_world().get_settings().fixed_delta_seconds
                frame_rate = 1 / delta_time
                sensor = ObjectsReader(vehicle, frame_rate)
